...
# Using built-in phsyics caused lag when too many stones on screen because every frame the
# physics were calculated -- whether falling or no. So just imitate the physics here only 
# when actually falling.
func _process(delta):
	if _is_falling:
		# Falling grav is less to give players a chance to react
		position.y += gravity * delta * 0.09
